...
class OrderCommentController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        // 閺屻儴顕楃拠銉吂閸楁洑绗呴幍鈧張澶婃櫌閸?        $orderGoods = OrderGoods::where('order_id', $order->id)->get();

        // 閺屻儴顕楀鑼剁槑娴犻娈?order_goods_id
        $commentedIds = Comment::where('order_id', $order->id)
            ->pluck('order_goods_id')
            ->toArray();

        // 鏉╁洦鎶ら幒澶婂嚒鐠囧嫪鐜惃?        $pendingGoods = $orderGoods->filter(function ($goods) use ($commentedIds) {
    ...
    public function submit(Request $request)
    {
        ...
        if (empty($form)) {
            return api_response(null, '鐠囧嫪鐜崘鍛啇不能为空', 500);
        }
