...
class RefundController extends Controller
{
    ...
    /**
     * 閼惧嘲褰囬崣顖滄暤鐠囧嘲鏁崥搴ｆ畱鐠併垹宕熼崯鍡楁惂鐠囷附鍎?     * 参数: orderGoodsId
     */
    public function goods(Request $request)
    {
        ...
        if (!$orderGoodsId) {
            return api_response(null, '鐠併垹宕熼崯鍡楁惂ID不能为空', 500);
        }
        ...
        if (!$order || $order->user_id != $user->id) {
            return api_response(null, '閺冪姵娼堥弻銉ф箙鐠囥儴顓归崡?, 400);
        }

        $totalPayPrice = (float) $order->pay_price;
        if ($order->total_price > 0) {
            // 閹稿鏅㈤崫浣风幆閺嶅吋鐦笟瀣瀻閹藉﹤鐤勬禒姗€鍣炬０?            $goodsTotalPrice = (float) $orderGoods->goods_price * $orderGoods->total_num;
    ...
    public function detail(Request $request)
    {
        ...
        if (!$refund) {
            return api_response(null, '閸烆喖鎮楅崡鏇氱瑝鐎涙ê婀?, 500);
        }
    ...
    public function delivery(Request $request)
    {
        ...
        return api_response(null, '閸欐垼鎻ｆ穱鈩冧紖瀹稿弶褰佹禍?);
    }
}